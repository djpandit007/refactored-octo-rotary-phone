...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRating = float(sum(ratings)) / totalRatings
    return (movieID, (totalRatings, avgRating))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda record: getCountsAndAverages(record))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (movieID, (movieName, (total, avg))): (avg, movieName, total)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avg, movieName, total): total > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda ((userID, movieID), (predictError, actualError)): math.pow(predictError - actualError, 2))
                       )
...
# TODO: Replace <FILL IN> with appropriate code
myModel = ALS.train(trainingRDD, 8, seed=seed, iterations=iterations, lambda_=regularizationParameter)
...
trainingAvgRatingCount = trainingRDD.count()
trainingAvgRating = (trainingRDD
                     .map(lambda (userID, movieID, rating): rating)
                     .reduce(lambda a, b: a + b))
trainingAvgRating /= trainingAvgRatingCount
...
myRatedMoviesRDD = sc.parallelize(myRatedMovies).map(lambda (userID, movieID, rating): (movieID, userID))
myUnratedMoviesRDD = (moviesRDD
                      .subtractByKey(myRatedMoviesRDD)
                      .map(lambda (movieID, movieName): (myUserID, movieID)))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (movieID, (totRatings, avgRating)): (movieID, totRatings))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (movieID, ((predRate, totRate), movieName)): (predRate, movieName, totRate)))
