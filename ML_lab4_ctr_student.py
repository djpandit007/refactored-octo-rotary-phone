...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    indValList = []
    for raw in rawFeats:
        indValList.append((OHEDict[raw], 1.0))
    return SparseVector(numOHEFeats, indValList)
...
def parsePoint(point):
    ...
    for num in range(len(features)):
        featVal.append((num, features[num]))
    return featVal
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    label = split[0]
    OHE = oneHotEncoding(rawFeats, OHEDict, numOHEFeats)
...
print trainingPredictions.take(1)
...
def evaluateResults(model, data):
    ...
    """
    #return getP(data.features, model.weights, model.intercept)
...
def parseHashPoint(point, numBuckets):
    ...
    label = point.split(',')[0]
    rawFeats = parsePoint(point)
    hashVal = SparseVector(numBuckets, hashFunction(numBuckets, rawFeats))
    return LabeledPoint(label, hashVal)
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda lp: float(len(lp.features.indices)) / d).mean()
