...
def simpleTokenize(string):
    ...
    """
    token = re.split(split_regex, string)    # Splits the string at whitespace
    token = map(lambda s: s.lower() , token) # Convert into lowercase
    token = filter(lambda s: s != '', token) # Remove the empty tokens
    return token
...
def tokenize(string):
    ...
    token = map(lambda s: s.lower() , token) # Convert into lowercase
    token = filter(lambda s: s != '', token) # Remove the empty tokens
    token = filter(lambda s: s not in stopwords, token) # Remove stopwords
    return token
...
def findBiggestRecord(vendorRDD):
    ...
    """
    biggestRecord = vendorRDD.takeOrdered(1, lambda (id, token): -1 * len(token))
...
def tf(tokens):
    ...
    for token in tokens:
        if token not in termFrequency.keys():
            termFrequency[token] = float(tokens.count(token)) / length
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (id, tokens): list(set(tokens))) # Remove duplicates from ONE document
    tokenCountPairTuple = uniqueTokens.map(lambda token: (token, 1.0)) # Token, Count tuple
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a, b: a + b) # Sum of occurences
    IDFvalues = tokenSumPairTuple.map(lambda (token, count): (token, N / count)) # Calculate IDF from given formula
    return IDFvalues
...
def tfidf(tokens, idfs):
    ...
    for tok in tfs:
        tfIdfDict[tok] = tfs[tok] * idfs[tok]
    return tfIdfDict
...
sims = similaritiesBroadcast.map(lambda (googleURL, amazonID, score): (amazonID + ' ' + googleURL, score))

trueDupsRDD = sims.join(goldStandard)
trueDupsCount = trueDupsRDD.count()

avgSimDups = (trueDupsRDD
              .map(lambda (s1, (score, s2)): score)
              .reduce(lambda a, b: a + b))
avgSimDups /= trueDupsCount

nonDupsRDD = sims.subtractByKey(goldStandard)
avgSimNon = (nonDupsRDD
             .map(lambda (s1, score): score)
             .reduce(lambda a, b: a + b))
avgSimNon /= nonDupsRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (id, weights): (id, norm(weights))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda (id, weights): (id, norm(weights))).collectAsMap()
...
def invert(record):
    ...
    """
    return [(vector, record[0]) for vector in record[1].keys()]
...
# TODO: Replace <FILL IN> with appropriate code
amazonWeightsDict = (amazonWeightsRDD
                     .map(lambda (id, weights): weights)
                     .flatMap(lambda x: x.items())
                     .collectAsMap())
amazonWeightsBroadcast = sc.broadcast(amazonWeightsDict)

googleWeightsDict = (googleWeightsRDD
                     .map(lambda (id, weights): weights)
                     .flatMap(lambda x: x.items())
                     .collectAsMap())
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for token in tokens:
        s += (amazonWeightsBroadcast.value[token] * googleWeightsBroadcast.value[token])
