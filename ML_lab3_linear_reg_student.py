...
def parsePoint(line):
    ...
    """
    split = line.split(',')
    label, features = split[0], split[1:]
    label = float(label)
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = rawData.map(lambda line: parsePoint(line))
onlyLabels = parsedDataInit.map(lambda lp: lp.label)
minYear = onlyLabels.takeOrdered(1)[0]
maxYear = onlyLabels.takeOrdered(1, lambda s: -1 * s)[0]
...
def calcRMSE(labelsAndPreds):
    ...
    return (np.sqrt(labelsAndPreds
                      .map(lambda (lab, pred): squaredError(lab, pred))
                      .mean()))
...
# TODO: Replace <FILL IN> with appropriate code
predTrainData = parsedTrainData.map(lambda lp: lp.label).mean()
labelsAndPredsTrain = parsedTrainData.map(lambda lp: (lp.label, predTrainData))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

predValData = parsedValData.map(lambda lp: lp.label).mean()
labelsAndPredsVal = parsedValData.map(lambda lp: (lp.label, predValData))
rmseValBase = calcRMSE(labelsAndPredsVal)

predTestData = parsedTestData.map(lambda lp: lp.label).mean()
labelsAndPredsTest = parsedTestData.map(lambda lp: (lp.label, predTestData))
...
def twoWayInteractions(lp):
    ...
    interact = []
    for element in itertools.product(features, features):
        interact.append(element[0] * element[1])
    features = np.append(features, interact)
    return LabeledPoint(label, features)
