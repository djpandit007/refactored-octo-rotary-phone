...
not200 = access_logs.filter(lambda error: error.response_code != 200)
...
dayHostCount = dayGroupedHosts.map(lambda (day, hostList): (day, len(hostList)))
...
daysWithHosts = dailyHosts.map(lambda (days, hosts): days).collect()
hosts = dailyHosts.map(lambda (days, hosts): hosts).collect()
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda (day, hostList): (day, len(hostList)))
                     .join(dailyHosts)
                     .map(lambda (day, (host, unique)): (day, host / unique))
                     .sortByKey()
                     .cache())
...
badRecords = (access_logs
              .filter(lambda error: error.response_code == 404)
             .cache())
